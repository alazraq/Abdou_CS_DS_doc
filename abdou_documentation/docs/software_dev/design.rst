...
according to the environment you are running it in.

- **Scala:** Article on loading configurations in Scala from a file, 
   from command line parameters or from environment variables is available 
   `here <https://danielasfregola.com/2015/06/01/loading-configurations-in-scala/>`__.
...
Please refer to :ref:`folder_organization`.

These conventions are the ones recommended by the official Java
...
-  Use the reversed Internet domain name of the company you are working
   for to begin their package names. For example,
   com.company_x.project_y.mypackage for a package named mypackage
   created by a programmer at working at company_x.com on project
   project_y.
